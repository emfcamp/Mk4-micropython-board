...
//
// Copyright (c) 2009-2018 Texas Instruments Incorporated.  All rights reserved.
...
static void postHandleDisconnect(void *pvMSCDevice);
static void postHandleEndpoints(void *pvMSCDevice, uint32_t ui32Status);
static void postConfigChangeHandler(void *pvMSCDevice, uint32_t ui32Value);
static void postHandleRequests(void *pvMSCDevice, tUSBRequest *psUSBRequest);
static void postHandleDevice(void *pvMSCDevice, uint32_t ui32Request,
                         void *pvRequestData);
                     ...
                     tCompositeEntry *psCompEntry)
{
    ...
    //
    psInst->pvMedia = 0; // psMSCDevice->sMediaFunctions.pfnOpen(0);
...
USBDSCSIRead10(tUSBDMSCDevice *psMSCDevice, tMSCCBW *psSCSICBW)
{
    ...
    if(psInst->pvMedia != 0)
    {
        ...
        {
            psMSCDevice->sMediaFunctions.pfnClose(psInst->pvMedia);
            psInst->pvMedia = 0;
        }
    }
...
USBDSCSIWrite10(tUSBDMSCDevice *psMSCDevice, tMSCCBW *psSCSICBW)
{
    ...
    if(psInst->pvMedia != 0)
    {
        ...
        // Configure and enable DMA for the OUT transfer.
        //
        psInst->psDMAInstance->ui32Complete = 0;
...
#define NUMMSGS 4u
#define STACKSIZE 1536u

#define REQUESTS 10u
#define ENDPOINTS 11u
#define DEVICE 12u
#define DISCONNECT 13u
#define CONFIG 14u
...
static void postHandleRequests(void *pvMSCDevice, tUSBRequest *psUSBRequest)
{
    ...
    msg.type = REQUESTS;
    msg.arg0 = pvMSCDevice;
    msg.arg1 = psUSBRequest;
    if (!Mailbox_post(mbox, &msg, 0)) {
        System_abort("Mailbox overflow in USB MSC");
    }
}

static void postHandleEndpoints(void *pvMSCDevice, uint32_t status)
{
    IsrMsg msg;

    msg.type = ENDPOINTS;
    msg.arg0 = pvMSCDevice;
    msg.arg1 = (void *)status;
    if (!Mailbox_post(mbox, &msg, 0)) {
        System_abort("Mailbox overflow in USB MSC");
    }
}
                             ...
                             void *pvRequestData)
{
    ...
    msg.type = DEVICE;
    msg.arg0 = pvMSCDevice;
    msg.arg1 = (void *)ui32Request;
    msg.arg2 = pvRequestData;
    if (!Mailbox_post(mbox, &msg, 0)) {
        System_abort("Mailbox overflow in USB MSC");
    }
}
...
static void postConfigChangeHandler(void *pvMSCDevice, uint32_t event)
{
    IsrMsg msg;
...
static void mscDaemon(UArg arg0, UArg arg1)
{
    IsrMsg msg;
    ...
    while (1) {
        ...
        switch (msg.type) {
        case REQUESTS:
            HandleRequests(msg.arg0, (tUSBRequest *)msg.arg1);
            break;

        case ENDPOINTS:
            HandleEndpoints(msg.arg0, (uint32_t)msg.arg1);
            break;

        case DEVICE:
            HandleDevice(msg.arg0, (uint32_t)msg.arg1, msg.arg2);
            break;

        case DISCONNECT:
            HandleDisconnect(msg.arg0);
            break;

        case CONFIG:
            ConfigChangeHandler(msg.arg0, (uint32_t)msg.arg1);
            break;

        default:
            System_printf("MSC bogus isr msg: %d\n", msg.type);
            break;
        }
    }
}
...
bool MSCD_setup(tUSBDMSCDevice * dev)
{
    bool status = false;
    ...
    if ((mbox = Mailbox_create(sizeof(IsrMsg), NUMMSGS, NULL, NULL))) {
        Task_Params params;

        Task_Params_init(&params);
        params.arg0 = (UArg)mbox;
        params.stackSize = STACKSIZE;
        params.priority = Task_numPriorities - 1;
        if ((task = Task_create(mscDaemon, &params, NULL))) {
            status = true;
        }
        else {
            Mailbox_delete(&mbox);
...
void MSCD_teardown(void)
{
    /* TODO: add a shutdown msg so task exits */
#if 0
    if (mbox) {
        Mailbox_delete(&mbox);
    }
